    ...
    <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '60px 20px', background: `linear-gradient(180deg, ${THEME_COLORS.sageBg} 0%, ${THEME_COLORS.white} 100%)` }}>
      ...
      <div style={{ width: '100%', maxWidth: '500px', backgroundColor: 'white', borderRadius: '24px', padding: '40px', boxShadow: '0 20px 40px rgba(0,0,0,0.05)', border: `1px solid ${THEME_COLORS.border}`, textAlign: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: '15px' }}>
          <Link
            to="/map"
            style={{ flex: 1, padding: '14px', backgroundColor: THEME_COLORS.sageDark, color: 'white', border: 'none', borderRadius: '12px', fontSize: '14px', fontWeight: '700', cursor: 'pointer', boxShadow: '0 4px 12px rgba(95, 126, 100, 0.25)', textDecoration: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
          >
            Haritaya Dön
          </Link>
          <button
            onClick={onLogout}
            style={{ padding: '14px 24px', backgroundColor: 'transparent', color: '#ef4444', border: '1px solid #ef4444', borderRadius: '12px', fontSize: '14px', fontWeight: '700', cursor: 'pointer' }}
          >
            Çıkış Yap
          </button>
        </div>
